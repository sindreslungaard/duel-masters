...
const (
	sundropArmorUID      = "a097478c-af87-4316-b42d-888418d07919"
	sundropArmorSetupSrc = "sundrop_armor_test_setup"
)

func TestSundropArmor(t *testing.T) {
	t.Run("printed characteristics", func(t *testing.T) {
		_, player, _ := setupDuel(t)

		spell, err := player.Player.SpawnCard(sundropArmorUID, match.HAND)
		require.NoError(t, err)

		assert.Equal(t, "Sundrop Armor", spell.Name)
	...
	t.Run("cannot choose itself, puts a different hand card into the shields, and goes to the graveyard", func(t *testing.T) {
		...
		spell := castSpell(t, scn, player, sundropArmorUID)
		...
		for _, offered := range action.Cards {
			assert.NotEqual(t, spell.ID, offered.CardID, "Sundrop Armor must not be an option for its own effect while it is resolving")
		}
		...
		assert.Equal(t, match.GRAVEYARD, spell.Zone, "a normal spell goes to the graveyard after resolving")
	...
	t.Run("with no other card in hand, the effect does nothing", func(t *testing.T) {
		scn, player, _ := setupDuel(t)

		emptyHand(t, player, sundropArmorSetupSrc)

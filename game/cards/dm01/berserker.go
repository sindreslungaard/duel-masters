	...
	c.Use(fx.Creature, func(card *match.Card, ctx *match.Context) {
		...
		if event, ok := ctx.Event.(*match.CardMoved); ok {
			...
			if event.CardID == card.ID && (event.To == match.BATTLEZONE || event.To == match.SPELLZONE) {
				...
				cards := match.SearchDeckForCnd(card.Player, ctx.Match, cnd.Spell, "Select 1 spell from your deck that will be shown to your opponent and sent to your hand", 1, 1, true)
